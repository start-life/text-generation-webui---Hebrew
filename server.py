...
def create_prompt_menus():
    with gr.Row():
        ...
        with gr.Column():
            with gr.Column():
                shared.gradio['save_prompt'] = gr.Button('שמור הנחיה')
...
def create_model_menus():
    with gr.Row():
        with gr.Column():
            with gr.Row():
                shared.gradio['model_menu'] = gr.Dropdown(choices=available_models, value=shared.model_name, label='מודל')
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                with gr.Column():
                    shared.gradio['custom_model_menu'] = gr.Textbox(label="LoRA הורדה מודל מותאם אישית או",
                                                                    info="היכנסו ל-Hugging Face שם משתמש/נתיב דגם, למשל: Facebook/Galactica-125M")
                with gr.Column():
                    shared.gradio['download_button'] = gr.Button("הורדה")
...
def create_settings_menus(default_preset):
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                shared.gradio['preset_menu'] = gr.Dropdown(choices=available_presets, value=default_preset if not shared.args.flexgen else 'Naive', label='פרמטרי יצירה מוגדרים מראש')
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Group():
                with gr.Row():
                    shared.gradio['add_bos_token'] = gr.Checkbox(value=shared.settings['add_bos_token'], label='Add the bos_token to the beginning of prompts', info='הפיכת אפשרות זו ללא זמינה יכולה להפוך את התשובות ליצירתיות יותר.')
                ...
                shared.gradio['truncation_length'] = gr.Slider(value=shared.settings['truncation_length'], minimum=shared.settings['truncation_length_min'], maximum=shared.settings['truncation_length_max'], step=1, label='קצץ את ההנחיה עד לאורך זה', info='האסימונים השמאליים ביותר מוסרים אם ההנחיה חורגת מהאורך הזה. רוב הדגמים דורשים שזה יהיה לכל היותר 2048.')
                shared.gradio['custom_stopping_strings'] = gr.Textbox(lines=1, value=shared.settings["custom_stopping_strings"] or None, label='מחרוזות עצירה מותאמות אישית', info='In addition to the defaults. Written between "" and separated by commas. For instance: "\\nYour Assistant:", "\\nThe assistant:"')
...
def create_interface():
    ...
    with gr.Blocks(css=ui.css if not shared.is_chat() else ui.css + ui.chat_css, analytics_enabled=False, title=title) as shared.gradio['interface']:
        if shared.is_chat():
            ...
            with gr.Tab("יצירת טקסט", elem_id="main"):
                ...
                shared.gradio['textbox'] = gr.Textbox(label='קלט')
                with gr.Row():
                    shared.gradio['Generate'] = gr.Button('ליצור', elem_id='Generate')
                    shared.gradio['Stop'] = gr.Button('עצור', elem_id="stop")
                with gr.Row():
                    shared.gradio['Regenerate'] = gr.Button('להתחדש - אם אתה לא מרוצה מהתשובה לחץ כאן לקבלת תשובה חדשה')
                    shared.gradio['Continue'] = gr.Button('להמשיך - את מתן התשובות לאחר עצירה')
                    shared.gradio['Impersonate'] = gr.Button('להתחזות - מתחזה אליך וכותב טקסט בשמך')
                with gr.Row():
                    shared.gradio['Send dummy message'] = gr.Button('שלח הודעת דמה - הקלד משהו בקלט וזה יופיע כקלט מדומה שלך, הצאטבוט לא יענה על זה')
                    shared.gradio['Send dummy reply'] = gr.Button('שלח תשובת דמה - הקלד משהו בקלט וזה יופיע כתשובת דמה של הצאטבוט')
                    shared.gradio['Replace last reply'] = gr.Button('החלף את התשובה האחרונה')
                    shared.gradio['Copy last reply'] = gr.Button('העתק את התשובה האחרונה')
                with gr.Row():
                    shared.gradio['Clear history'] = gr.Button('נקה היסטוריה')
                    shared.gradio['Clear history-confirm'] = gr.Button('לאשר', variant="stop", visible=False)
                    shared.gradio['Clear history-cancel'] = gr.Button('לבטל', visible=False)
                    shared.gradio['Remove last'] = gr.Button('הסר אחרון - הסר את התשובה האחרונה')

                shared.gradio["mode"] = gr.Radio(choices=["cai-chat", "chat", "instruct"], value="cai-chat", label="מצב")
            ...
            with gr.Tab("דמות", elem_id="chat-settings"):
                with gr.Row():
                    with gr.Column(scale=8):
                        shared.gradio['name1'] = gr.Textbox(value=shared.settings['name1'], lines=1, label='השם שלך')
                        shared.gradio['name2'] = gr.Textbox(value=shared.settings['name2'], lines=1, label='שם הדמות')
                        shared.gradio['greeting'] = gr.Textbox(value=shared.settings['greeting'], lines=4, label='ברכה - (המשפט בו פותח הצאטבוט את השיחה)')
                    ...
                    with gr.Column(scale=1):
                        shared.gradio['character_picture'] = gr.Image(label='תמונת דמות', type="pil")
                        shared.gradio['your_picture'] = gr.Image(label='התמונה שלך', type="pil", value=Image.open(Path("cache/pfp_me.png")) if Path("cache/pfp_me.png").exists() else None)
                with gr.Row():
                    shared.gradio['character_menu'] = gr.Dropdown(choices=available_characters, value='None', label='דמות', elem_id='character-menu')
                    ui.create_refresh_button(shared.gradio['character_menu'], lambda: None, lambda: {'choices': get_available_characters()}, 'refresh-button')

                with gr.Row():
                    with gr.Tab('הסטוריית צאט'):
                        with gr.Row():
                            with gr.Column():
                                gr.Markdown('להעלות')
                            ...
                            with gr.Column():
                                ...
                                shared.gradio['download_button'] = gr.Button(value='לחץ עלי')
                    with gr.Tab('Upload character'):
                        gr.Markdown("# JSON פורמט")
                        with gr.Row():
                            with gr.Column():
                                gr.Markdown('1. JSON-בחר את קובץ ה')
                                shared.gradio['upload_json'] = gr.File(type='binary', file_types=['.json'])
                            with gr.Column():
                                gr.Markdown('2. בחר את תמונת הפרופיל של הדמות שלך (אופציונלי)')
            ...
            with gr.Tab("פרמטרים", elem_id="parameters"):
                with gr.Box():
                    gr.Markdown("Chat parameters")
                    with gr.Row():
                        with gr.Column():
                            shared.gradio['max_new_tokens'] = gr.Slider(minimum=shared.settings['max_new_tokens_min'], maximum=shared.settings['max_new_tokens_max'], step=1, label='מקסימום אסימונים חדשים', value=shared.settings['max_new_tokens'])
                            shared.gradio['chat_prompt_size'] = gr.Slider(minimum=shared.settings['chat_prompt_size_min'], maximum=shared.settings['chat_prompt_size_max'], step=1, label='גודל מספר מרבי של אסימונים', value=shared.settings['chat_prompt_size'])
                        with gr.Column():
                ...
                with gr.Row():
                    with gr.Column(scale=4):
                        ...
                        with gr.Row():
                            with gr.Column():
                                with gr.Row():
                                    shared.gradio['Generate'] = gr.Button('ליצור')
                                    shared.gradio['Stop'] = gr.Button('עצור')
                            with gr.Column():
                                pass
            ...
            with gr.Tab("יצירת טקסט", elem_id="main"):
                with gr.Row():
                    with gr.Column():
                        shared.gradio['textbox'] = gr.Textbox(value=default_text, lines=21, label='קלט')
                        shared.gradio['max_new_tokens'] = gr.Slider(minimum=shared.settings['max_new_tokens_min'], maximum=shared.settings['max_new_tokens_max'], step=1, label='מקסימום אסימונים חדשים', value=shared.settings['max_new_tokens'])
                        shared.gradio['Generate'] = gr.Button('ליצור')
                        with gr.Row():
                            with gr.Column():
                                shared.gradio['Continue'] = gr.Button('להמשיך')
                            with gr.Column():
                                shared.gradio['Stop'] = gr.Button('עצור')
                    ...
                    with gr.Column():
                        with gr.Tab('Raw'):
                            shared.gradio['output_textbox'] = gr.Textbox(lines=27, label='פלט')
                        with gr.Tab('Markdown'):
                            shared.gradio['markdown'] = gr.Markdown()
                        with gr.Tab('HTML'):
                            shared.gradio['html'] = gr.HTML()
        ...
        with gr.Tab("מודל", elem_id="model-tab"):
            create_model_menus()

        with gr.Tab("אימון", elem_id="training-tab"):
            training.create_train_interface()

        with gr.Tab("מצב ממשק", elem_id="interface-mode"):
            modes = ["בררת מחדל", "notebook", "chat", "cai_chat"]
            ...
            gr.Markdown("*ניסיוני*")
            shared.gradio['interface_modes_menu'] = gr.Dropdown(choices=modes, value=current_mode, label="מצב")
            shared.gradio['extensions_menu'] = gr.CheckboxGroup(choices=get_available_extensions(), value=shared.args.extensions, label="הרחבות זמינות")
            shared.gradio['bool_menu'] = gr.CheckboxGroup(choices=bool_list, value=bool_active, label="דגלי שורת פקודה בוליאניים")
            shared.gradio['reset_interface'] = gr.Button("החל והפעל מחדש את הממשק")
